...
#define ProgramMemStart           0x00001400 //Beginning of application program memory (not occupied by bootloader).
                                             //**THIS VALUE MUST BE ALIGNED WITH BLOCK BOUNDRY**
                                             //Also, in order to work correctly, make sure the StartPageToErase is set to erase this section.
...
#define MaxPageToEraseWithConfigs 42         //Page 170 contains the flash configurations words on the PIC24FJ256GB110.
                                             //Page 170 is also smaller than the rest of the (1536 byte) pages.
...
#define ProgramMemStopWithConfigs 0x0000ABF8 //Must be instruction word aligned address.  This address does not get updated,
                                             //but the one just below it does: IE: If AddressToStopPopulating = 0x200, 0x1FF
                                             //is the last programmed address (0x200 not programmed)	
...
#define	QUERY_DEVICE              0x02 //Command that the host uses to learn about the device (what regions can be programmed,
                                       //and what type of memory is the region)
#define	UNLOCK_CONFIG             0x03 //Note, this command is used for both locking and unlocking the config bits
                                       //(see the "//Unlock Configs Command Definitions" below)
...
#define	PROGRAM_COMPLETE          0x06 //If host send less than a RequestDataBlockSize to be programmed,
                                       //or if it wished to program whatever was left in the buffer, it uses this command.
#define GET_DATA                  0x07 //The host sends this command in order to read out memory from the device.
                                       //Used during verify (and read/export hex operations)
#define	RESET_DEVICE              0x08 //Resets the microcontroller, so it can update the config bits
                                       //(if they were programmed, and so as to leave the bootloader
                                       //(and potentially go back into the main application)
...
#define	TypeProgramMemory         0x01 //When the host sends a QUERY_DEVICE command, need to respond by populating a list of valid memory regions that
                                       //exist in the device (and should be programmed)
...
#define WORDSIZE                  0x02 //PIC18 uses 2 byte instruction words, PIC24 uses 3 byte "instruction words"
                                       //(which take 2 addresses, since each address is for a 16 bit word;
                                       //the upper word contains a "phantom" byte which is unimplemented.).
...
void sendSpiLED(BYTE msb0, BYTE lsb0, BYTE msb1, BYTE lsb1);
BOOL checkPressPads(void);
...
#endif
{
  ...
  SPI1CON1 = 0x053f;//sy 16mhz   /* Initalizes the spi module */
  SPI1CON2 = 0x0000;
...
void BootApplication(void)
{
  ...
  {	
    ...
    case ERASE_DEVICE:
    {
      ...
      NVMCONbits.WREN = 0; //Good practice to clear WREN bit anytime we are not expecting to do erase/write operations,
    ...
    case PROGRAM_DEVICE:
    {
      ...
      if(ProgrammedPointer == (unsigned long)PacketFromPC.Address)
      {
        for(i = 0; i < (PacketFromPC.Size/WORDSIZE); i++)
        {
          ...
          ProgrammingBuffer[BufferedDataIndex] = PacketFromPC.Data[(RequestDataBlockSize-PacketFromPC.Size)/WORDSIZE+i];
          BufferedDataIndex++;
          ProgrammedPointer++;
      ...
      //else host sent us a non-contiguous packet address...  to make this firmware simpler,
      //host should not do this without sending a PROGRAM_COMPLETE command in between program sections.
      BootState = IdleState;
    }
    break;
    ...
    {
      WriteFlashSubBlock();
      ProgrammedPointer = InvalidAddress; //Reinitialize pointer to an invalid range,
                                          //so we know the next PROGRAM_DEVICE will be the start address of a contiguous section.
      BootState = IdleState;
    }
    break;
    case GET_DATA:
    {
      if(!USBHandleBusy(USBInHandle))
      {
        ...
        for(i = 0; i < (PacketFromPC.Size/2); i=i+2)
        {
          ...
          PacketToPC.Data[RequestDataBlockSize/WORDSIZE + i + 1 - PacketFromPC.Size/WORDSIZE] = FlashMemoryValue.word.HW;
        }
...
void initLedDriver(void)
{
  sendSpiLED(0x0B, 0x07, 0x0B, 0x07); // Scan Limit full range
  sendSpiLED(0x0A, 0x0F, 0x0A, 0x0F); // Max Intensity 0x00[min] - 0x0F[max]
 ...
 // print startup pattern 
  ...
  sendSpiLED(0x0C, 0x01, 0x0C, 0x01); // Shutdown Normal Operation
  sendSpiLED(0x0F, 0x00, 0x0F, 0x00); // Display Test Off
}
...
BOOL checkPressPads(void)
{
  ...
  for(i = 0; i < 8; i++)
  {
    ...
    for(j = 0; j < 8; j++)
    {
      if(i == 0 && j ==0 && SR_QH == 0)
        num++;
      else if(i == 0 && j ==1 && SR_QH == 0)
        num++;
      else if(i == 0 && j ==2 && SR_QH == 0)
        num++;
